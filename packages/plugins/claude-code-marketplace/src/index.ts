...
// Inlined types from @devtime/shared (no external dependencies for marketplace plugin)
const API_VERSION = 'v1';

const TOOLS = ['vscode', 'claude-code', 'codex', 'zed'] as const;
const ACTIVITY_TYPES = ['coding', 'debugging', 'prompting', 'browsing'] as const;
...
function createHeartbeat(input: HookInput): Heartbeat {
...
function queueHeartbeat(heartbeat: Heartbeat): void {
...
async function main(): Promise<void> {
  ...
  if (command === 'flush') {
    ...
    // Being called as a hook, read from stdin
